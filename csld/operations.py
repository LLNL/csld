...
class SymmOp():
    ...
    def __init__(self, affine_transformation_matrix, rot, tr, atm_f, rot_f, tr_f, tol=0.01):
        """
        ...
        atm_f = np.array(atm_f)
        if atm_f.shape != (4, 4):
            raise ValueError("Affine Matrix must be a 4x4 numpy array!")
        self.atm_f = atm_f
        self.rot = np.array(rot)
        self.tr = tr
        self.rot_f = np.array(rot_f, dtype=np.intc)
        self.tr_f = tr_f
        self.tol = tol

    @classmethod
    def from_rotation_and_translation(cls,rotation_matrix=((1, 0, 0),
                                                       (0, 1, 0),
                                                       (0, 0, 1)),
        ...
        Args:
            ...
            rot_frac, trans_frac: the counterparts in fractional coordinates
        ...
        atm_f[0:3][:, 0:3] = rot_f
        atm_f[0:3][:, 3] = tr_f
        return cls(affine_matrix, rotation_matrix, translation_vec, atm_f, rot_f, tr_f, tol)
    ...
    def operate(self, point, coords_frac=False):
        """
        ...
        if coords_frac:
            return np.dot(self.atm_f, affine_point)[0:3]
        else:
            return np.dot(self.affine_matrix, affine_point)[0:3]
    
    def operate_multi(self, points, coords_frac=False):
        """
        ...
        if coords_frac:
            return np.inner(affine_points, self.atm_f)[:, :-1]
        else:
