...
class Lattice():
    ...
    @property
    def WS_radius(self):
        return np.power(self.volume*3/(4*np.pi),1./3)
    ...
    def get_all_distances(self, fcoords1, fcoords2, nogamma=False):
        """
        ...
        images = pbc_images(nogamma=nogamma)
    ...
    def vector_in_wigner_seitz(self, vec, tol=1E-10):
        images = pbc_images(nogamma=True) 
#np.delete(pbc_images(), 13, 0)
        vec_len=np.linalg.norm(vec) + tol
        for pt in self.get_cartesian_coords(images):
           if np.linalg.norm(vec+pt) < vec_len:
               return False
        return True
