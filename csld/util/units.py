...
from .physical_constants import N_a, e
import re
...
bar_to_eVang = 6.24150913E-7
kBbyeV = 8.61733238E-5
eVAng_to_THz= 15.6333046
...
_UNAME2UTYPE = {}
for utype, d in list(ALL_UNITS.items()):
    assert not set(d.keys()).intersection(list(_UNAME2UTYPE.keys()))
...
def _get_si_unit(unit):
    ...
    si_unit = [k for k in list(BASE_UNITS[unit_type].keys()) if BASE_UNITS[unit_type][k] == 1]
...
class Unit(collections.Mapping):
    ...
    def __init__(self, unit_def):
        ...
        if isinstance(unit_def, str):
            ...
            for m in re.finditer("([A-Za-z]+)\s*\^*\s*([\-0-9]*)", unit_def):
                ...
                k = m.group(1)
                for utype in list(ALL_UNITS.values()):
                    for u in list(utype.keys()):
                        if u.lower() == k.lower():
                            k = u
                unit[k] += p
        else:
            unit = {k: v for k, v in list(dict(unit_def).items()) if v != 0}

        def check_mappings(u):
            for v in list(DERIVED_UNITS.values()):
    ...
    def __pow__(self, i):
        return Unit({k: v * i for k, v in list(self.items())})
    ...
    def __repr__(self):
        sorted_keys = sorted(list(self._unit.keys()),
    ...
    def as_base_units(self):
        ...
        for k, v in list(self.items()):
            derived = False
            for d in list(DERIVED_UNITS.values()):
                if k in d:
                    for k2, v2 in list(d[k].items()):
            ...
            if not derived:
                ...
                b[si] += v
                factor *= f ** v
        return {k: v for k, v in list(b.items()) if v != 0}, factor
    ...
    def get_conversion_factor(self, new_unit):
        ...
        units_new = sorted(list(un_base.items()),
                           key=lambda d: _UNAME2UTYPE[d[0]])
        units_old = sorted(list(uo_base.items()),
                           key=lambda d: _UNAME2UTYPE[d[0]])
...
def obj_with_unit(obj, unit):
    ...
    if isinstance(obj, numbers.Number):
        ...
        return {k: obj_with_unit(v, unit) for k,v in list(obj.items())}
    else:
