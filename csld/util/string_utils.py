...
def fn_available(filename):
    import os.path
    if not os.path.exists(filename):
        return filename
    else:
        filenum = 1
        fn=filename+"-"+str(filenum)
...
def str2arr(s, typ=float, shape=None):
    m = np.array(list(map(typ, s.split())))
    if shape is None:
        return m
    else:
        return m.reshape(shape)

def setting2arr(setting, key, shape=None, defaultval=None):
    import os.path
    val = setting.get(key, defaultval)
    if val is None:
        return None
    if os.path.isfile(val):
        return np.loadtxt(val).reshape(shape)
    else:
        try:
            return str2arr(val).reshape(shape)
        except:
            print("array file %s not found" %(val))
            return None

def str2bool(v):
    return v.lower() in ("yes", "y", "true", "t", "1", '.true.', '.t.', 'on')
