...
from .util.periodic_table import Element, Specie, DummySpecie, get_el_sp
    ...
    def __init__(self, atoms_n_occu, coords, properties=None):
        ...
        Args:
            atoms_n_occu: Species on the site. Can be:
                ...
                iii. Just empty string, no messing with composition stuff
            coords: Cartesian coordinates of site.
            properties: Properties associated with the site as a dict, e.g.
                {"magmom": 5}. Defaults to None.
        """
        if isinstance(atoms_n_occu, str) and atoms_n_occu == '':
# if I just do NOT want to mess with composition stuff
            self._species = ''
            self._is_ordered = True
        elif issubclass(atoms_n_occu.__class__, collections.Mapping):
    ...
    def dist(p0, p1):
        ...
        Returns:
            Distance (float)
        """
        return np.linalg.norm(p1.coords - p0.coords)
    ...
    def __lt__(self, other):
        ...
        if self._species.average_electroneg != other._species.average_electroneg:
            return self._species.average_electroneg<other._species.average_electroneg
...
class PeriodicSite(Site):
    ...
    def __init__(self, atoms_n_occu, coords, lattice, to_unit_cell=False,
        ...
        if to_unit_cell:
            ...
            self._fcoords[np.where(self._fcoords>1-1e-12)]=0
    ...
    def distance_and_image(self, other, jimage=None):
        ...
        Returns:
            ...
            of the other site for which the distance applies.
        """
        return self.distance_and_image_from_frac_coords(other._fcoords if isinstance(other,PeriodicSite) else other, jimage)
    ...
    def distance(self, other, jimage=None):
        ...
        Args:
            other (PeriodicSite) OR frac_coords: Other site to get distance from.
            jimage (3x1 array): Specific periodic image in terms of lattice
                translations, e.g., [1,0,0] implies to take periodic image
                that is one a-lattice vector away. If jimage == None,
                the image that is nearest to the site is found.
    ...
    def __str__(self):
        return "{:2} {:8.5f} {:8.5f} {:8.5f}".format(self.species_string, *self._fcoords.tolist())
    ...
    def set_coords(self, coords, cart=True):
        """
        set coordinates
        :param coords:
        :param cart: whether input coordinates are cartesian
        :return:
        """
        if cart:
            self._coords = coords
            ...
            self._fcoords = coords
            self._coords = self._lattice.get_cartesian_coords(coords)

    def move_by(self, dcoords, cart=True):
        self.set_coords(dcoords+(self._coords if cart else self._fcoords), cart)
