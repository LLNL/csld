...
class BasicLatticeModel(AtomicModel):
    ...
    def generate_clusters(cls, prim, **kwargs):
        ...
        Iterative generation starting from empty and point clusters
        ...
        for ord in range(maxord+1):
            ...
            print('length of clus_per_ord : ',len(clus_per_ord))
        return cls(prim, raw, **kwargs)
    ...
    def translate_to_supercell(self, sc, orb_idx=None, clus_idx=None):
        ...
        :return: all clusters in the supercell. Each cluster is given as
        ...
        orb_range = orb_idx if orb_idx is not None else range(len(self.orbits)) # number of orbits
        allclus = []
        for iorb in orb_range:  # loop over all orbits
            orbit = self.orbits[iorb]  # save orbit
            clus_range = clus_idx if clus_idx is not None else range(len(orbit.clusters))  # number of clusters in the orbit
            for ic in clus_range: # loop over all clusters in the orbit
                clus = orbit.clusters[ic]  # save cluster
                ig = orbit.clusters_ig[ic]  # save space group operations
#                print('Space Group operation index \n',ig)
                allclus.extend([cx, iorb, ig, ic] for cx in self.translate_cluster_to_supercell(sc, clus)) # over all translated clusters
        return allclus
    ...
    def translate_cluster_to_supercell(sc, clus):
        ...
        """
        if clus.order == 0:  # if the order is 0
            return [[]]*sc.n_cell # nothing to do..
        coords= clus.frac_coords # the cluster in fractional coordinates
        use_compiled_code = True
        if use_compiled_code:
            from f_util import f_util
            allclus = f_util.tr_cl_sc(sc._ijkl.T, sc.sc_mat.T, sc.inv_sc_mat.T, sc.sc_ref.T, sc.prim.frac_coords.T, clus._ijkls_np.T).T-1 # get all clusters
        else:
